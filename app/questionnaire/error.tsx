...
export default function QuestionnaireError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { Sentry.captureException(error) }, [error])

  return (
    <div className="min-h-screen bg-stone-50 dark:bg-gray-950 flex items-center justify-center p-6">
      <div className="max-w-sm text-center">
        <h2 className="font-display text-2xl text-stone-900 dark:text-stone-100 mb-3">Questionnaire error</h2>
        <p className="text-stone-500 dark:text-stone-400 text-sm mb-6">Something went wrong. Your previous answers are saved.</p>
