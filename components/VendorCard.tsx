...
export function VendorCard({ vendor, isSelected, onToggleSelect, matchScore, matchReasons }: VendorCardProps) {
  ...
  return (
    <div className={`group relative bg-white/80 backdrop-blur-md rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border ${isSelected ? 'border-rose-300 ring-2 ring-rose-200' : 'border-white/50 hover:border-rose-200'}`}>
      {/* Match score badge */}
      {matchScore && (
        ...
        <button
          ...
          className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all duration-300 ${
            isSelected
              ...
              : 'bg-white/80 backdrop-blur-sm border-gray-300 hover:border-rose-400'
          }`}
        >
          {isSelected && (
      ...
      <div className="relative h-48 bg-gradient-to-br from-rose-100 via-pink-50 to-purple-100 overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="mb-3">
          ...
          <h3 className="text-xl font-serif font-medium text-gray-900 group-hover:text-rose-600 transition-colors">
            {vendor.name}
          </h3>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-3">
          ...
          <span className="font-light">{vendor.suburb || vendor.location}, {vendor.state}</span>
        </div>
        ...
        <div className="flex items-baseline gap-2 mb-4">
          ...
          {vendor.priceRange && (
            <span className="text-xs text-gray-500">
              {'$'.repeat(['BUDGET', 'MODERATE', 'PREMIUM', 'LUXURY'].indexOf(vendor.priceRange) + 1)}
            </span>
          )}
        </div>
        ...
        {vendor.rating && (
          <div className="flex items-center gap-1 mb-4">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className={`w-4 h-4 ${i < Math.floor(vendor.rating!) ? 'text-rose-400' : 'text-gray-200'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
            ...
            <span className="ml-2 text-sm text-gray-600 font-light">{vendor.rating.toFixed(1)}</span>
          </div>
        )}
        ...
        {vendor.styles.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {vendor.styles.slice(0, 3).map((style, idx) => (
              <span
                key={idx}
                className="px-2 py-1 text-xs bg-rose-50 text-rose-700 rounded-lg font-light"
              >
                {style}
              </span>
            ))}
        ...
        <button
          ...
          className="w-full py-2 text-sm font-medium text-rose-600 hover:text-rose-700 transition-colors flex items-center justify-center gap-2 group/btn"
        >
