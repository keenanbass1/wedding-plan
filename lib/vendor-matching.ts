...
export async function findMatchingVendors(
  ...
  // Group by category
  const venues = scoredVendors
    .filter(v => v.category === 'VENUE')
    .slice(0, 5);

  const photographers = scoredVendors
    .filter(v => v.category === 'PHOTOGRAPHER')
    .slice(0, 5);

  const caterers = scoredVendors
...
function calculateMatchScore(
  ...
  if (requirements.style && vendor.styles) {
    const styleMatch = vendor.styles.some(s =>
